export const run = {
  ...
  run: async (m, { prefix, command, args }) => {
    const validModes = Object.keys(global.db.groups[m.chat])
      .filter(key => typeof global.db.groups[m.chat][key] === 'boolean')

    if (args.length < 1) {
      let modeList = 'Current settings:\n'
      for (const mode of validModes) {
        modeList += `- ${mode}: ${global.db.groups[m.chat][mode] ? 'on' : 'off'}\n`
      }
