  ...
  init: async function (ctx) {
    if (ctx.sender && ctx.sender.endsWith('@s.whatsapp.net')) {
      if (!this.users[ctx.sender]) {
        this.users[ctx.sender] = { ...models.users, name: ctx.pushName, totalChat: 1 }
      } else {
        ...
        Object.keys(models.users).forEach(key => {
          if (typeof this.users[ctx.sender][key] === 'undefined') {
            this.users[ctx.sender][key] = models.users[key]
          }
        })
      }
    }
    ...
    if (ctx.isGroup) {
      if (!this.groups[ctx.chat]) {
        this.groups[ctx.chat] = { ...models.groups }
      } else {
        Object.keys(models.groups).forEach(key => {
          if (typeof this.groups[ctx.chat][key] === 'undefined') {
            this.groups[ctx.chat][key] = models.groups[key]
          }
        })
      }
    }
