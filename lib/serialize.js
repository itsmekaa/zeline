import { getContentType, jidNormalizedUser, jidDecode, downloadMediaMessage } from 'baileys'
...
export const serialize = async (raw, sock) => {
  ...
  if (raw.message) {
        ...
        const mapLid = (jid, targetContent) => {
          if (jid.endsWith('@lid')) {
            ...
            if (member && member.phoneNumber) {
              const lidNum = jidDecode(jid)?.user
              const phoneNum = jidDecode(member.phoneNumber)?.user
    ...
    m.prefix = prefix.find(p => m.body.startsWith(p)) || ''
    ...
    m.isOwner = owner.includes(jidDecode(m.sender)?.user) || raw.key.fromMe
    ...
    if (m.isGroup && groupParticipants.length > 0) {
      try {
        ...
        const botNumberJid = process.env.BOT ? `${process.env.BOT}@s.whatsapp.net` : ''
        const botIds = [
          botIdNormalized,
          (jidDecode(botIdNormalized)?.user || '') + '@s.whatsapp.net',
          (jidDecode(botIdNormalized)?.user || '') + '@lid',
          botNumberJid
        ].filter(Boolean)
    ...
    if (quotedMsg) {
      ...
      qm.prefix = prefix.find(p => qm.body.startsWith(p)) || ''
