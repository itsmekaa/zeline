...
export const serialize = async (m, sock) => {
  ...
  ctx.id = m.key.id
  ctx.timestamps = Number(m.messageTimestamp) * 1000 || Date.now()
  ...
  if (ctx.isGroup) {
    groupParticipants = (await sock.groupMetadata(ctx.chat).catch(() => null))?.participants ?? []
  }
  ...
  if (m.message) {
    ...
    if (ctx.isGroup && hasLid && groupParticipants.length > 0) {
      try {
        const mapLidStr = (jid) => {
    ...
    if (ctx.isGroup && groupParticipants.length > 0) {
      try {
        const admins = []
